  ...
  const loadAppointments = async () => {
    try {
      ...
      try {
        ...
        throw new Error(
          `Unexpected response (${response.status}): ${text?.slice(0, 120)}`
        );
      }
    ...
    <>
      ...
      <div className="profile-menu visible">
        ...
        <div className="profile-body">
          <p>
            <strong>Welcome,</strong> {user?.fullname || "Guest"}
          </p>
          ...
          <ul>
            {loading ? (
              ...
              appointments.map((apt, index) => (
                <li key={index}>
                  <strong>{apt.service}</strong> - {apt.appointment_date} at{" "}
                  {apt.time}
                  <br />
                  ...
                  {(() => {
                    try {
                      const aptDt = new Date(
                        `${apt.appointment_date}T${apt.time}:00`
                      );
                      const now = new Date();
                      const cancellable =
                        apt.status !== "Cancelled" &&
                        apt.status !== "Completed" &&
                        apt.status !== "Abandoned" &&
                        apt.status !== "Done" &&
                        aptDt > now;
                      if (cancellable) {
                        return (
                            ...
                            onClick={async () => {
                              ...
                              try {
                                const res = await fetch(
                                  `${API_BASE_URL}/api/appointments/${apt.id}/cancel`,
                                  {
                                    method: "POST",
                                    credentials: "include",
                                  }
                                ...
                                if (res.ok) {
                                  ...
                                  alert(
                                    "❌ Failed to cancel appointment: " + t
                                  );
                                }
                              } catch (e) {
